...
import { Download, Upload, Code, FileText, Settings, User, MessageSquare, ChevronDown, ChevronRight } from 'lucide-react';
...
link.click();`;
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        const validMessages = parsed.filter(
          item => item && typeof item === 'object' && 'time' in item && 'sender' in item && 'content' in item,
        );

        setParsedData(validMessages);

        // sender별 통계 및 아바타 정보 생성 - 개선된 데이터 구조 대응
        const senderStats: { [key: string]: { count: number; avatarUrl?: string } } = {};
        validMessages.forEach(msg => {
          ...
          if (sender.includes('http') || sender.includes('www.') || sender.includes('.com')) {
          ...
          if ((msg as any).messageType && (msg as any).messageType.includes('desc')) {
            sender = 'SYSTEM';
          }

          if (!senderStats[sender]) {
            senderStats[sender] = { count: 0, avatarUrl: (msg as any).avatarUrl || '' };
          }
          ...
          .map(([sender, data]) => {
            ...
            if (data.avatarUrl) {
              try {
                ...
                if (url.hostname.includes('d20.io') || url.hostname.includes('files.d20.io')) {
                  const parts = pathname.split('/');
                  const filename = parts[parts.length - 2] || parts[parts.length - 1]; // 마지막 또는 그 전 부분
  ...
  const updateSenderType = (sender: string, type: string) => {
    setSenderMappings(prev => prev.map(mapping => (mapping.sender === sender ? { ...mapping, type } : mapping)));
  };

  const updateSenderImage = (sender: string, imageFile: string) => {
    setSenderMappings(prev => prev.map(mapping => (mapping.sender === sender ? { ...mapping, imageFile } : mapping)));
  };

  const toggleSenderExpanded = (sender: string) => {
    setSenderMappings(prev =>
      prev.map(mapping => (mapping.sender === sender ? { ...mapping, expanded: !mapping.expanded } : mapping)),
    );
  };
  ...
  const applyMappings = () => {
    const mappingDict = senderMappings.reduce((acc, mapping) => {
    ...
    const enhancedData = parsedData.map(message => {
      ...
      if ((message as any).messageType && (message as any).messageType.includes('desc')) {
        sender = 'SYSTEM';
      }
  ...
  const convertToLogEntries = (): LogEntry[] => {
    return parsedData.map((message, index) => {
      const logEntry = {
        type: message.type === 'character' ? 'character' : message.type === 'system' ? 'system' : 'ooc',
  ...
  const getCharactersFromData = () => {
    const characterSenders = senderMappings
      ...
      .map(mapping => {
        ...
        if (mapping.avatarUrl && !mapping.avatarUrl.includes('$0') && mapping.avatarUrl.trim() !== '') {
  ...
  const saveMappingPreset = () => {
    const preset = {
      name: prompt('매핑 프리셋 이름을 입력하세요:') || `프리셋_${new Date().toLocaleDateString()}`,
      mappings: senderMappings,
      createdAt: new Date().toISOString(),
    };

    const existingPresets = JSON.parse(localStorage.getItem('trpg_mapping_presets') || '[]');
    existingPresets.push(preset);
    localStorage.setItem('trpg_mapping_presets', JSON.stringify(existingPresets));
  ...
  const loadMappingPreset = () => {
    ...
    const presetNames = existingPresets
      .map(
        (preset: any, index: number) => `${index + 1}. ${preset.name} (${new Date(preset.createdAt).toLocaleDateString()})`,
      )
      .join('\n');

    const selection = prompt(`불러올 프리셋을 선택하세요 (번호 입력):\n${presetNames}`);
    ...
    if (selectedIndex >= 0 && selectedIndex < existingPresets.length) {
      ...
      const newMappings = senderMappings.map(current => {
        const presetMapping = selectedPreset.mappings.find((preset: any) => preset.sender === current.sender);
        return presetMapping ? { ...current, type: presetMapping.type } : current;
      });
    ...
    <div className="container mx-auto p-6 max-w-6xl">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">
          Roll20이나 다른 플랫폼에서 추출한 채팅 로그를 분석하고 JSON 형태로 변환합니다.
        </p>
      </div>
      ...
      <Tabs value={currentTab} onValueChange={setCurrentTab} className="space-y-6">
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="mapping" className="flex items-center gap-2" disabled={senderMappings.length === 0}>
            <Settings className="h-4 w-4" />
            타입 매핑
          </TabsTrigger>
          <TabsTrigger value="result" className="flex items-center gap-2" disabled={parsedData.length === 0}>
        ...
        <TabsContent value="script">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Roll20 웹페이지에서 개발자 도구 콘솔에 붙여넣어 실행하는 스크립트입니다.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="relative">
                ...
                <Button onClick={copyScriptToClipboard} className="absolute top-2 right-2" size="sm">
                  복사
                </Button>
              </div>
              <div className="p-4 bg-blue-50 dark:bg-blue-950/20 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <div className="mt-3 p-2 bg-amber-50 dark:bg-amber-950/20 rounded border border-amber-200 dark:border-amber-800">
                  <p className="text-xs text-amber-700 dark:text-amber-300">
                    <strong>개선사항:</strong> DOM 구조를 정확히 분석하여 메시지를 추출하고, 아바타 이미지를 자동으로
                    수집하며, 귓속말과 일반 메시지를 구분합니다.
                  </p>
                </div>
        ...
        <TabsContent value="parse">
          <Card>
            <CardHeader>
              ...
              <CardDescription>브라우저에서 추출한 JSON 데이터를 여기에 붙여넣거나 파일로 업로드하세요.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              {rawData && (
                <div className="text-sm text-muted-foreground">입력된 데이터: {rawData.length.toLocaleString()} 문자</div>
              )}
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="mapping">
          <Card>
            <CardHeader>
              ...
              <CardDescription>각 발신자의 메시지 타입을 설정하세요. 메시지 개수순으로 정렬되어 있습니다.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {senderMappings.length > 0 && (
                <>
                  <div className="flex justify-between items-center">
                    <div className="text-sm text-muted-foreground">총 {senderMappings.length}명의 발신자</div>
                    <div className="flex gap-2">
                      <Button onClick={loadMappingPreset} variant="outline" size="sm">
                        불러오기
                      </Button>
                      <Button onClick={saveMappingPreset} variant="outline" size="sm">
                        저장하기
                      </Button>
                      <Button onClick={applyMappings} className="flex items-center gap-2">
                      ...
                      <div key={mapping.sender} className="border rounded-lg">
                        <div className="flex items-start gap-3 p-3">
                          <div className="flex items-center gap-3 flex-1">
                            {/* 아바타 미리보기 */}
                            {!mapping.avatarUrl || mapping?.avatarUrl?.startsWith('https://app.roll20.net') ? (
                            ...
                            <div className="flex-1">
                              <div className="font-medium flex items-center gap-2 mb-1">
                                ...
                                <Button
                                  ...
                                  onClick={() => toggleSenderExpanded(mapping.sender)}
                                >
                                  {mapping.expanded ? (
                          ...
                          <div className="flex flex-col gap-2">
                            <Select value={mapping.type} onValueChange={value => updateSenderType(mapping.sender, value)}>
                            ...
                            {mapping.type === 'character' && (
                              <div className="flex flex-col gap-1">
                                <Label className="text-xs text-muted-foreground">이미지 파일명</Label>
                                <Input
                                  placeholder="character-name.png"
                                  value={mapping.imageFile || ''}
                                  onChange={e => updateSenderImage(mapping.sender, e.target.value)}
                                  className="w-48 text-sm"
                                />
                                <div className="text-xs text-muted-foreground">public/assets/ 폴더 기준</div>
                                {mapping.avatarUrl && (
                                  <Button
                                    type="button"
                                    variant="ghost"
                                    ...
                                    onClick={() => {
                                      if (mapping.avatarUrl) {
                                        navigator.clipboard.writeText(mapping.avatarUrl);
                          ...
                          <div className="border-t bg-gray-50 dark:bg-gray-900/20 p-3">
                            ...
                            <div className="space-y-2 max-h-48 overflow-y-auto">
                              {getSenderMessages(mapping.sender, 5).map((msg, msgIndex) => (
                                <div key={msgIndex} className="p-2 bg-white dark:bg-gray-800 rounded text-xs">
                                  <div className="text-muted-foreground mb-1">{(msg as any).time || '시간 없음'}</div>
        ...
        <TabsContent value="result">
          <Card>
            <CardHeader>
              ...
              <CardDescription>파싱된 메시지들을 스크립트 형태로 확인하고 JSON으로 다운로드할 수 있습니다.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {parsedData.length > 0 ? (
                <>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Badge variant="secondary">총 {parsedData.length}개의 메시지</Badge>
                      <Badge variant="outline">{senderMappings.filter(m => m.type === 'character').length}명의 캐릭터</Badge>
                    </div>
                    <Button onClick={downloadParsedData} className="flex items-center gap-2">
                      <Download className="h-4 w-4" />
                      JSON 다운로드
                    </Button>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="p-3 bg-gray-50 dark:bg-gray-900 rounded-lg text-xs">
                      ...
                      <div className="space-y-1">
                        ...
                        <div>로그 엔트리 수: {convertToLogEntries().length}</div>
                        <div className="max-h-32 overflow-y-auto">
                          <strong>캐릭터 목록:</strong>
                          {getCharactersFromData().map(char => {
                            const mapping = senderMappings.find(m => m.sender === char.name);
                            return (
